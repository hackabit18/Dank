...
//echo "fsafsafsa";*/
fwrite($myfile, explode("###@@@#@",$_POST["topic"])[0]);
fclose($myfile);
$myfile = fopen("input.txt", "w") or die("Unable to write file!");
//$txt = "Jsdassdohn Doe\n";
//fwrite($myfile, $txt);
//$txt = "Jane Doe\n";
//echo shell_exec("cd /var/www/html && python new.py");
//echo "fsafsafsa";*/
fwrite($myfile, explode("###@@@#@",$_POST["topic"])[1]);
fclose($myfile);

exec("cd /var/www/html/ && python3 new.py");
...
//echo $myarr[1];
$part1= $_POST["topic"]."@@@###Input Text\n"."uuuuu".join($myarr);
$part1=$part1."tttttttttt";
...
$myfile3= fopen("error.txt", "r") or die("Unable to write file!");

$er_r="";
if ($myfile3) {
    while (($linee = fgets($myfile3)) !== false) {
        $er_r=$er_r.$linee;
    }
